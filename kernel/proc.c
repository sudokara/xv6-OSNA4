...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    struct proc *firstP = 0; // earliest created process
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
      if (p->state == RUNNABLE)
      {
        if (firstP == 0 || p->creation_time < firstP->creation_time) // pick earliest created process
        {
          ...
          {
            release(&firstP->lock); // release lock since new process found
          }
          firstP = p; // new first created process
          continue;   // do not release lock of the process to be executed
        }
      }
      release(&p->lock);
    }
    ...
    p = firstP;
    if (p > 0) // execute if a process is found
    {
      // Switch to chosen process.  It is the process's job
      // to release its lock and then reacquire it
      // before jumping back to us.
